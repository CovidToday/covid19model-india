...
df = get_data.getJHU(countryname,thresh=100,end="04/19/2020")
N = get_data.getPopulationData(countryname)

a_i = float(sys.argv[3])     # relative infectivity of infected who are asymptomatics
p_a = float(sys.argv[1])      # fraction of infected who are asymptomatics
f_a  = float(sys.argv[2])     # fraction of asymtomatics that get detected

folder = '/a_i_{}_p_a_{}_f_a_{}/'.format(a_i,p_a,f_a)
if not os.path.exists(countryname+folder):
    os.makedirs(countryname+folder)
    os.makedirs(countryname+folder+"bootstrap/")

predictionsize = 1                                 # Leave Last 'predictionsize' days out
...
g = 1/gs[0]                                          # Mean Gamma
d_a = 1/d_as[0]                                      # recovery rate for Asymptomatics
...
try:
    ...
    print(popt)
#    print(df[["active","recovered","dead"]][:-predictionsize].values.dtype)
except:
    print("failed")
    exit()
...
for i in range(n_bootstrap):
    ...
    d_a = 1/d_as[i+1]
    resample = np.random.poisson(Po_mean)
#    print(resample.shape,resample.dtype)
    try:
        popt,pcov = curve_fit(curvefit_model,time,resample.flatten(),p0=guess,method='trf',bounds=param_bounds,sigma=data_weightage)
#        print(popt)
#   except:
#       print("failed")
#   popt,pcov = curve_fit(curvefit_model,time,df[["active","recovered","dead"]][:-predictionsize].values.flatten(),P0=guess,method='trf',bounds=param_bounds,sigma=data_weightage)
        popts.append(popt)
        np.save(countryname+folder+"bootstrap/optimal_prediction_{}.npy".format(i),model(np.arange(0,df.shape[0]+365,0.1),*popt)[::10,:])
    except:
        fit_fail+=1
#        print(fit_fail)
    print("Bootstrap iteration: {} of {}".format(i+1,n_bootstrap-fit_fail),end="\r")
